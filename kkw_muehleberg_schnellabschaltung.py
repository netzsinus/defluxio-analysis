...
# Global parameters
starttime = 1436173380 - 2*60
bottomtime = 1436173380 + 5*60
startfreq = 49.996
bottomfreq = 49.948
setfreq = 50.0
...
with pd.get_store(args.datafile) as store:
  muehleberg = store['muehleberg']
  #TODO: Move these timestamps to stagedata.py
  #muehleberg = muehleberg[(muehleberg.unix >= 1427268780) &
  #    (muehleberg.unix < 1427269200)]
  ...
  ax.set_ylim(ylim)
  xlim = (np.min(muehleberg.time), np.max(muehleberg.time))
  ax.set_xlim(xlim)
  ax.plot(muehleberg.time, muehleberg.freq, 'b-', label=r"Netzfrequenz")
  #ax.plot(muehleberg.time, muehleberg.freq_sg, 'b', label=r"Netzfrequenz")
  ...
  ax[1].set_xlim((np.min(muehleberg_momentum_df.s_since_midnight.astype(int)), 
    np.max(muehleberg_momentum_df.s_since_midnight.astype(int))))
  ax[1].plot(muehleberg_momentum_df.s_since_midnight.astype(int),
      muehleberg_momentum_df.momentum, 'r-', label="Momentum")
  #ax.legend()
  #hfmt = dates.DateFormatter('%H:%M')
  ...
  f.suptitle(u"Schnellabschaltung KKW Mühleberg 06.07.2015 - Leistungsgradienten")
  f.autofmt_xdate()
  plt.savefig("images/muehleberg-gradienten.png")#, bbox_inches='tight')
